...
import { SuppliersInterface } from "../models/ISupplier";
...
function ProductStockCreate() {
  ...
  const [productStock, setProductStock] = useState<Partial<ProductStockInterface>>({});
  ...
  const handleInputChange = (
    ...
    const id = event.target.id as keyof typeof ProductStockCreate;
 
    const { value } = event.target;
 
    setProductStock({ ...productStock, [id]: value });
  ...
  const getSupplier = async () => {
    fetch(`${apiUrl}/suppliers`, requestOptions)
      .then((response) => response.json())
      .then((res) => {
        if (res.data) {
  ...
  function submit() {
    let data = {
      ...
      Price: convertType(productStock.Price),
      Amount: convertType(productStock.Amount),
      SupplierID: convertType(productStock.SupplierID),
      StaffID: convertType(staffs?.ID),
      ProductTime: selectedDate,
    };
      ...
      <Paper className={classes.paper}>
        ...
        <Grid container spacing={3} className={classes.root}>
         ...
         <Grid item xs={2}>
          ...
          <Grid item xs={6}>
            <FormControl fullWidth variant="outlined">
              ...
              >
                ...
                {suppliers.map((item:SuppliersInterface) => (
                  <option value={item.ID} key={item.ID}>
                    {item.Name}
                  </option>
                ))}
          ...
          <Grid item xs={6}>
            <FormControl fullWidth variant="outlined">
              <p>พนักงานเพิ่มสินค้า</p>
              <Select
                native
                disabled
                value={productStock.StaffID}
                /*onChange={handleChange}
                inputProps={{
                  name: "StaffID",
                }}*/
